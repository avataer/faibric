...
async function run() {
    log("=== COFFEE SHOP v8 - COMPLETE TEST WITH CHAT ITERATION ===");
...
Include espresso, latte, cappuccino with prices.`);
        ...
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "v8-01-initial.png"), fullPage: true });
        log("Screenshot: v8-01-initial.png - Initial build");

        // STEP 2: Chat amendment
        log("STEP 2: Sending chat amendment for color changes...");
        const chatInput = await page.locator("textarea").first();
        await chatInput.fill("Make all section backgrounds use bg-amber-50 (cream color). The header should be bg-amber-900 (dark brown). NO gray colors anywhere.");
        await page.keyboard.press("Enter");
        log("Amendment sent via chat");
        ...
        while (rebuilding && Date.now() - rebuildStart < 180000) {
            const deployed = await page.locator("button:has-text(\"Deployed\")").first();
            if (await deployed.isVisible().catch(() => false)) {
                rebuilding = false;
            } else {
                await page.waitForTimeout(3000);
                log(`Rebuilding... ${Math.round((Date.now() - rebuildStart) / 1000)}s`);
            }
        }
        await page.waitForTimeout(5000);

        // STEP 4: Screenshot showing chat + preview
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "v8-02-chat-amendment-with-preview.png"), fullPage: true });
        log("CRITICAL Screenshot: v8-02-chat-amendment-with-preview.png - Chat panel with amendment + preview");
        ...
        if (url) {
            ...
            fs.writeFileSync(path.join(TEST_DIR, "deployment_url_v8.txt"), url);
        }
        ...
        if (url) {
            ...
            await page2.screenshot({ path: path.join(SCREENSHOTS_DIR, "v8-03-deployed.png"), fullPage: true });
            log("Screenshot: v8-03-deployed.png - Deployed site");

            await browser2.close();
        }

        log("=== TEST v8 COMPLETE ===");
        return { success: true, url };

    } catch (e) {
        log(`ERROR: ${e.message}`);
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "v8-error.png") }).catch(() => {});
        await browser.close();
        throw e;
    }
}
