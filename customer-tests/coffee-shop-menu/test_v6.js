...
async function run() {
    log('=== COFFEE SHOP v6 - DEBUG COLOR ENFORCEMENT ===');
    ...
    try {
        ...
        // Use explicit keywords that trigger color enforcement
        log('Submitting with explicit COFFEE BROWN CREAM keywords...');
...
Content: Coffee drinks menu with prices.`);
        ...
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v6-01-initial.png'), fullPage: true });
        ...
        if (url) {
            ...
            fs.writeFileSync(path.join(TEST_DIR, 'deployment_url_v6.txt'), url);
            ...
            await page2.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v6-02-deployed.png'), fullPage: true });
            log('Screenshot: Deployed site');

            await browser2.close();
        }

        log('=== TEST v6 COMPLETE ===');
        return { success: true, url };

    } catch (e) {
        log(`ERROR: ${e.message}`);
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v6-error.png') }).catch(() => {});
        await browser.close();
        throw e;
    }
}
