...
async function run() {
    log('=== COFFEE SHOP v3 - WITH FIXED PROMPTS ===');
...
Content: Menu with espresso, lattes, cappuccinos. Daily specials. About section.`);
        ...
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v3-01-initial.png'), fullPage: true });
        log('Screenshot: Initial build');
        
        // Send amendment
        log('Sending color amendment...');
        const input = await page.locator('input[type="text"]:not([disabled])').first();
        await input.fill('Make the header bg-amber-900 (dark brown) and all sections bg-amber-50 (cream). NO GRAY.');
        await input.press('Enter');
        
        await page.waitForTimeout(60000); // Wait for rebuild
        
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v3-02-amendment.png'), fullPage: true });
        ...
        if (url) {
            ...
            fs.writeFileSync(path.join(TEST_DIR, 'deployment_url_v3.txt'), url);
            ...
            await page2.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v3-03-deployed.png'), fullPage: true });
        ...
        log(`ERROR: ${e.message}`);
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'v3-error.png') }).catch(() => {});
        await browser.close();
        throw e;
    }
}
