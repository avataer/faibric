...
class AdminFeedbackQuestion(models.Model):
    """
    Questions generated by LLM for admin to answer.
    Admin reviews happen AFTER items are live, not as a gate.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    library_item = models.ForeignKey(
        LibraryItem, 
        on_delete=models.CASCADE, 
        related_name='feedback_questions'
    )
    ...
    # What action to take based on answer
    QUESTION_TYPES = [
        ('quality', 'Rate quality (1-5)'),
        ('like_dislike', 'Like or dislike specific aspect'),
        ('improvement', 'Suggest improvement'),
        ('categorization', 'Confirm categorization'),
    ]
    question_type = models.CharField(max_length=20, choices=QUESTION_TYPES, default='quality')
    
    # Was this feedback applied?
