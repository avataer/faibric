...
"""
    ...
    # IMPORTANT: Components defined OUTSIDE App to prevent re-creation on render
    admin_panel_code = '''
  // FAIBRIC ADMIN - State
  ...
  const [adminAuth, setAdminAuth] = React.useState(
    !!localStorage.getItem("faibric_admin_token")
  );
  const [adminView, setAdminView] = React.useState("builder");
  const [previewUrl] = React.useState(
    window.location.origin + window.location.pathname.replace("/faibric", "").replace(/\\/$/, "")
  );
'''

    admin_login_view = '''
  // Admin Login - UNCONTROLLED input to prevent focus issues
  const passwordInputRef = React.useRef(null);
  const [loginError, setLoginError] = React.useState("");
  
  const doLogin = () => {
    const pass = passwordInputRef.current?.value || "";
    const stored = localStorage.getItem("faibric_admin_pass") || "faibric123";
    if (pass === stored) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md">
        ...
        <div className="space-y-4">
          ...
          <button onClick={doLogin} className="w-full py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700">
            Login
          </button>
...
'''
  ...
  // Chat state for builder
  const [chatMsgs, setChatMsgs] = React.useState([
    { from: "ai", text: "Welcome to Faibric Builder! How would you like to modify your app?" }
  ]);
      ...
      <nav className="bg-gray-900 text-white p-3 flex-shrink-0">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={doLogout} className="text-sm text-red-400 hover:text-red-300">Logout</button>
          </div>
        </div>
      </nav>
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="w-1/3 bg-white border-r flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {building && <div className="bg-gray-100 p-3 rounded-lg w-fit"><span className="text-gray-500 text-sm">Building...</span></div>}
            </div>
            <div className="p-4 border-t">
              <div className="flex gap-2">
                ...
                <button onClick={sendChat} disabled={building}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:bg-gray-300">Send</button>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => { localStorage.clear(); alert("Cache cleared!"); }} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">Clear Cache</button>
              </div>
            </div>
          </div>
        </main>
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="bg-white p-4 rounded-lg shadow text-center">
                <p className="text-3xl font-bold text-blue-600">{localStorage.getItem("faibric_views") || "0"}</p>
                <p className="text-gray-500 text-sm">Page Views</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow text-center">
                <p className="text-3xl font-bold text-green-600">{localStorage.getItem("faibric_sessions") || "0"}</p>
                <p className="text-gray-500 text-sm">Sessions</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow text-center">
                <p className="text-3xl font-bold text-purple-600">{localStorage.getItem("faibric_api_calls") || "0"}</p>
                <p className="text-gray-500 text-sm">API Calls</p>
              </div>
            </div>
            <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow space-y-4">
              <div>
                ...
                <select defaultValue={refreshInterval} onChange={(e) => updateRefreshInterval?.(parseInt(e.target.value))} className="w-full p-2 border rounded">
              ...
              <div className="pt-4 border-t">
                ...
                <input type="password" placeholder="New password" 
                  onBlur={(e) => { if (e.target.value) { localStorage.setItem("faibric_admin_pass", e.target.value); e.target.value = ""; alert("Password updated!"); } }}
